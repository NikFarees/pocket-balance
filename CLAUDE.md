...
- **Tools** (`src/lib/ai/tools.ts`): **read** tools (`get_today_status`, `get_month_summary`, `get_backup_balance`, `get_debts_summary`, `get_notes`, `find_entries`) and **write** tools — full CRUD: `add_*`/`update_*`/`delete_*` for `expense`, `income`, `debt`, `backup_transaction`, `investment_transaction`. `WRITE_TOOL_NAMES`/`READ_TOOL_NAMES` drive the route's behaviour. `find_entries` looks up rows (and their ids) so the model can target updates/deletes.
