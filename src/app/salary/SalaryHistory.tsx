        ...
        {paged.map((s) => (
          ...
          >
            <div className="min-w-0">
              <p className="font-medium text-sm">{format(parseISO(s.month), 'MMMM yyyy')}</p>
              {s.notes && <p className="text-xs text-muted-foreground mt-0.5">{s.notes}</p>}
            </div>
            <div className="flex items-center gap-2">
              <p className="font-semibold text-sm">RM {Number(s.amount).toLocaleString('en-MY', { minimumFractionDigits: 2 })}</p>
