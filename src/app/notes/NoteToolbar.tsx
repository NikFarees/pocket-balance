...
import {
  ...
  Quote, Strikethrough, Table as TableIcon, Trash2, Underline as UnderlineIcon,
} from 'lucide-react'
...
export function NoteToolbar({ editor }: { editor: Editor }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-0.5 border-b p-1">
      ...
      <Button type="button" variant="ghost" size="sm" className="h-8 w-8 p-0" disabled={!s.inTable} onClick={() => editor.chain().focus().deleteTable().run()} aria-label="Delete table"><Trash2 className="size-4" /></Button>
    </div>
  )
}
